...
// Explain the differences between .map, .reduce and .filter and describe a use case for each.

// 2. Higher Order Function is a function that receives other functions, and Callback Function is a function that is passed into another function as an argument

// 3. A closure is when an inner function reaches out of its scope to grab a variable defined in an outer function

// Describe the four principles of the 'this' keyword.
...
function animalNames(zooAnimals) {
  const displayNames = zooAnimals.map(function (zooAnimal) { return (`name: ${zooAnimal.animal_name}, scientific: ${zooAnimal.scientific_name}`) })
  return displayNames
}
...
function lowerCaseNames(zooAnimals) {
  const names = zooAnimals.map(function (item) { return (`${item.animal_name.toLowerCase()}`) })
  return names
}
...
function lowPopulationAnimals(zooAnimals) {
  const lowpop = zooAnimals.filter(function (item) { return (item.population < 5) })
  return lowpop
}
...
function USApop(zooAnimals) {
  const localpop = zooAnimals.reduce(function (currentTotal, item) {
    return (currentTotal + item.population)
  }, 0)
  return localpop
}
